...
PR_CHECK_KEEP_RAW_SANCOV="${PR_CHECK_KEEP_RAW_SANCOV:-0}"
...
Options:
  ...
  --keep-raw-sancov          Keep baseline/raw_sancov after each check
                             (default: remove to save disk space)
...
cleanup_raw_sancov() {
    ...
    if [[ "${PR_CHECK_KEEP_RAW_SANCOV}" -eq 1 ]]; then
        return 0
    fi

    local removed=0
    while IFS= read -r -d '' raw_dir; do
        echo "Removing raw_sancov artifacts: ${raw_dir}"
        rm -rf "$raw_dir"
        removed=1
    done < <(find "$output_dir" -type d -name 'raw_sancov' -print0 2>/dev/null)

    if [[ "$removed" -eq 1 ]]; then
        echo "Freed disk space under ${output_dir} (processed_sancov and CSVs retained)."
    fi
}
