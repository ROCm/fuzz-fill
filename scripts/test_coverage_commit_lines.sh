...
# End-to-end: LIT baseline symcov -> diff added-lines -> commit-lines uncovered list.
...
ADDED_LINES_DIR=$OUTPUT_DIR/diff_added_lines
...
# 1) Baseline suite coverage (required symcov under TEST_SUITE_OUTPUT_DIR/processed_sancov/)
...
# 2) Lines added in COMMIT (same tree as --llvm-repo)
